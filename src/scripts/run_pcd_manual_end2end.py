...
def run_end_to_end(
    ...
    SI_esp = normalize_vec(ceig) * normalize_vec(bext)
...
def main():
    ...
    for ipath in inputs:
        ...
        # Top-k by SH* (esp variant)
        df_rank = df_nodes.copy()
        df_rank = df_rank.sort_values('SH_star_esp', ascending=False)
        topk = int(max(1, args.topk))
        topk_df = df_rank.head(topk)[['node', 'cluster', 'SH_star_esp', 'SH_star_deg', 'SI_esp', 'Bext', 'C_eig', 'U', 'typology']]
        ...
        print(f"Top-{topk} por SH*_esp guardado en: {topk_path}")
